...
class IndexController extends WebInfoController {
    public function index(){
        ...
        $data1=$m->where($where)->order('utime desc')->field("productId,name,money,smoney,num,productImg,utime")->select();
        $this->assign('data1',$data1);
        //小学课程
        $where=array('prodid'=>C('PRODID'),'cateId'=>'Gz4i45','state'=>5);
        $data2=$m->where($where)->order('utime desc')->field("productId,name,money,smoney,num,productImg,utime")->select();
        $this->assign('data2',$data2);
        //中学课程
        $where=array('prodid'=>C('PRODID'),'cateId'=>'ZsuP8M','state'=>5);
        $data3=$m->where($where)->order('utime desc')->field("productId,name,money,smoney,num,productImg,utime")->select();
        $this->assign('data3',$data3);
        //作业托管
        $where=array('prodid'=>C('PRODID'),'cateId'=>'UgTQqC','state'=>5);
        $data4=$m->where($where)->order('utime desc')->field("productId,name,money,smoney,num,productImg,utime")->select();
        $this->assign('data4',$data4);
        //兴趣拓展
        $where=array('prodid'=>C('PRODID'),'cateId'=>'BDSkvu','state'=>5);
        $data5=$m->where($where)->order('utime desc')->field("productId,name,money,smoney,num,productImg,utime")->select();
