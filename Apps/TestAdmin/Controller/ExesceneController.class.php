...
class ExesceneController extends CommonController{
    public function index(){
	    ...
	    $stagetesterid=I('stagetesterid',$data[0]['id']);	
	    
	    $where=array("stagetesterid"=>$stagetesterid);
	    $exe=D('tp_exescene')->where($where)->order("sn")->select();
	    $this->assign('exe',$exe);
    ...
    public function test(){
        ...
        $where=array("proid"=>I('proid',$data[0]['proid']),"stagetesterid"=>$stagetesterid,"type"=>I('type'));
        $this->assign('w',$where);
   ...
   public function fail(){
    ...
    public function insert(){
        $sceneid=I('sceneid');
        $data=M('tp_scene')->field("type,level,swho,swhen,testip,scene,flow")->find($sceneid);
